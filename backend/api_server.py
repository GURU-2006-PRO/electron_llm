...
if llm_available:
    try:
        ...
        if OPENROUTER_API_KEY:
            ...
            print("[INFO] OpenRouter API key not set (optional)")
        
        if not GEMINI_3_FLASH_KEY:
            print("[WARNING] GEMINI_API_KEY_3_FLASH not set in .env file")
            print("[WARNING] Please add your Gemini API key to use advanced features")
        else:
            print("[OK] Gemini 3.0 Flash key configured")
        
        if GEMINI_2_5_FLASH_1_KEY:
            print("[OK] Gemini 2.5 Flash #1 key configured")
        ...
        print(f"[ERROR] Could not initialize Multi-LLM service: {e}")
        import traceback
        traceback.print_exc()
        llm_service = None
        gemini_manager = None
